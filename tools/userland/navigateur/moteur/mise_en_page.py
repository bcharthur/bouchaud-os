...
class Contexte:
    ...
    def __init__(self, regles, largeur_page):
        self.regles = regles
        self.largeur_page = largeur_page


def construit(racine, regles, largeur_page):
    """Construit l'arbre de boites et rend (racine, hauteur totale)."""
    contexte = Contexte(regles, largeur_page)
    style_initial = {
...
def _dispose_ligne(boite, nœuds, x, y, largeur, style_parent, contexte):
    ...
    while pile:
        ...
        if nœud.balise == "img":
            curseur_x, curseur_y, hauteur_ligne_courante = _pose_texte(
                boite, nœud.attributs.get("alt", "[image]"), style_enfant,
                lien_enfant, x, largeur, curseur_x, curseur_y,
                hauteur_ligne_courante)
            continue
