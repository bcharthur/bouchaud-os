...
if (($MinWidth -eq 0) -xor ($MinHeight -eq 0)) { throw "MinWidth et MinHeight doivent etre tous deux nuls ou tous deux non nuls" }
...
if (-not $Output) { $Output = Join-Path $RepoRoot "target\reference\bouchaud-reference-stage2.img" }
$Output = [System.IO.Path]::GetFullPath($Output)
if ($Ramdisk) {
    $Ramdisk = [System.IO.Path]::GetFullPath($Ramdisk)
    if (-not (Test-Path -LiteralPath $Ramdisk -PathType Leaf)) { Fail "ramdisk Stage 2 absent: $Ramdisk" }
}
if ($PrebootShim) {
...
try {
    if ($Ramdisk -and $MinWidth -gt 0 -and $PrebootShim) {
        Write-Host "STAGE2_GOP_PREBOOT_REQUEST=${MinWidth}x${MinHeight}"
        Write-Host "STAGE2_RAMDISK_REQUEST=$Ramdisk"
        Write-Host "STAGE2_PREBOOT_SHIM=$PrebootShim"
        & cargo +nightly-2026-06-01 run --release --target-dir "$BuilderTarget" -- "$Kernel" "$Output" "$MinWidth" "$MinHeight" "$Ramdisk" "$PrebootShim"
    }
    elseif ($Ramdisk -and $MinWidth -gt 0) {
        Write-Host "STAGE2_GOP_MIN_REQUEST=${MinWidth}x${MinHeight}"
        Write-Host "STAGE2_RAMDISK_REQUEST=$Ramdisk"
        & cargo +nightly-2026-06-01 run --release --target-dir "$BuilderTarget" -- "$Kernel" "$Output" "$MinWidth" "$MinHeight" "$Ramdisk"
    }
    elseif ($Ramdisk) {
        Write-Host "STAGE2_RAMDISK_REQUEST=$Ramdisk"
        & cargo +nightly-2026-06-01 run --release --target-dir "$BuilderTarget" -- "$Kernel" "$Output" "$Ramdisk"
    }
    elseif ($MinWidth -gt 0) {
        Write-Host "STAGE2_GOP_MIN_REQUEST=${MinWidth}x${MinHeight}"
        & cargo +nightly-2026-06-01 run --release --target-dir "$BuilderTarget" -- "$Kernel" "$Output" "$MinWidth" "$MinHeight"
    }
    else {
...
if (-not (Test-Path -LiteralPath $Output -PathType Leaf)) { Fail "image Stage 2 absente: $Output" }
...
if ($MinWidth -gt 0) { Write-Host "STAGE2_GOP_MIN=${MinWidth}x${MinHeight}" }
if ($PrebootShim) { Write-Host "BOUCHAUD_STAGE2_UEFI_PREBOOT_PROBE_READY" }
if ($Ramdisk) {
