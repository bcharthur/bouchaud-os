//! SHA-512/SHA-384 minimal (FIPS 180-4), utilise pour ECDSA P-384.
...
const H384: [u64; 8] = [0xcbbb9d5dc1059ed8,0x629a292a367cd507,0x9159015a3070dd17,0x152fecd8f70e5939,0x67332667ffc00b31,0x8eb44a8768581511,0xdb0c2e0d64f98fa7,0x47b5481dbefa4fa4];
const H512: [u64; 8] = [0x6a09e667f3bcc908,0xbb67ae8584caa73b,0x3c6ef372fe94f82b,0xa54ff53a5f1d36f1,0x510e527fade682d1,0x9b05688c2b3e6c1f,0x1f83d9abfb41bd6b,0x5be0cd19137e2179];

fn digest(data: &[u8], mut h: [u64; 8]) -> [u8; 64] {
    ...
    for block in msg.chunks(128) {
        let mut w = [0u64; 80];
        for i in 0..16 { w[i] = u64::from_be_bytes([block[8*i],block[8*i+1],block[8*i+2],block[8*i+3],block[8*i+4],block[8*i+5],block[8*i+6],block[8*i+7]]); }
        for i in 16..80 {
        ...
        for i in 0..80 {
            let s1=e.rotate_right(14)^e.rotate_right(18)^e.rotate_right(41); let ch=(e&f)^((!e)&g);
            let t1=hh.wrapping_add(s1).wrapping_add(ch).wrapping_add(K[i]).wrapping_add(w[i]);
            let s0=a.rotate_right(28)^a.rotate_right(34)^a.rotate_right(39); let maj=(a&b)^(a&c)^(b&c);
            let t2=s0.wrapping_add(maj); hh=g; g=f; f=e; e=d.wrapping_add(t1); d=c; c=b; b=a; a=t1.wrapping_add(t2);
        }
        h[0]=h[0].wrapping_add(a); h[1]=h[1].wrapping_add(b); h[2]=h[2].wrapping_add(c); h[3]=h[3].wrapping_add(d);
        h[4]=h[4].wrapping_add(e); h[5]=h[5].wrapping_add(f); h[6]=h[6].wrapping_add(g); h[7]=h[7].wrapping_add(hh);
    }
    let mut out=[0u8;64]; for i in 0..8 { out[8*i..8*i+8].copy_from_slice(&h[i].to_be_bytes()); } out
}

pub fn sha512(data: &[u8]) -> [u8; 64] { digest(data, H512) }
pub fn sha384(data: &[u8]) -> [u8; 48] { let d = digest(data, H384); let mut out=[0u8;48]; out.copy_from_slice(&d[..48]); out }

pub fn selftest() -> Result<(), &'static str> {
    let d = sha384(b"abc");
    let want: [u8;48] = [0xcb,0x00,0x75,0x3f,0x45,0xa3,0x5e,0x8b,0xb5,0xa0,0x3d,0x69,0x9a,0xc6,0x50,0x07,0x27,0x2c,0x32,0xab,0x0e,0xde,0xd1,0x63,0x1a,0x8b,0x60,0x5a,0x43,0xff,0x5b,0xed,0x80,0x86,0x07,0x2b,0xa1,0xe7,0xcc,0x23,0x58,0xba,0xec,0xa1,0x34,0xc8,0x25,0xa7];
    if d != want { return Err("sha384(abc)"); }
    if sha512(b"abc")[..4] != [0xdd,0xaf,0x35,0xa1] { return Err("sha512(abc)"); }
    Ok(())
}
